...
def create_context_extractor(config: dict):
    ...
    if extractor_type == 'cnn':
        defaults = {
            'cnn_filters': 16, 'cnn_kernel_size': 3, 'cnn_layers': 2,
    ...
    if extractor_type == 'rnn':
        defaults = {
            'rnn_units': 32, 'rnn_layers': 2, 'rnn_type': 'gru',
...
from forecast_pipeline.config import kpa2psi, INITIAL_PRESSURE
# =============================================================================
...
class PhysicsInformedForecaster(layers.Layer):
    ...
    def _init_physics(self):
        ...
        dataset_name = self.strategy_cfg.get("dataset_name")
...
def create_model(
    ...
    # This will now be correctly defined with a known shape
    ...
    # 1. Create the context extractor instance
    ctxt_extractor = create_context_extractor(dict(extractor_config))
        
    # 2. Call the extractor
    ctxt_vec = ctxt_extractor(data_feats)

    extractor_type = extractor_config.get('type', '').lower()
    if extractor_type == 'tcn':
        context_dim = extractor_config.get('tcn_filters')
    elif extractor_type == 'cnn':
        context_dim = extractor_config.get('cnn_filters')
    elif extractor_type == 'rnn':
        rnn_units = extractor_config.get('rnn_units')
        context_dim = rnn_units * 2 if extractor_config.get('bidirectional') else rnn_units
    elif extractor_type == 'aggregate':
        context_dim = len(extractor_config.get('feature_indices', (0, 1))) * 2
    elif extractor_type == 'identity':
        mode = extractor_config.get('mode', 'flatten').lower()
        if mode == 'flatten':
            context_dim = len(DATA_IDX) * timesteps
        elif mode == 'last_step':
            context_dim = len(DATA_IDX)
        elif mode == 'none':
            context_dim = 1 # CORRECT: 'none' mode outputs a single dimension
        else:
            context_dim = len(DATA_IDX) * timesteps
            
    else:
        raise ValueError(f"Unknown extractor type for context_dim calculation: {extractor_type}")
        
    # Now, we use a Lambda layer to set the static shape of the context vector.
    # This explicitly tells the Keras graph the exact shape of this tensor.
    ctxt_vec.set_shape([None, context_dim])
